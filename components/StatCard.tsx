...
const StatCard: React.FC<StatCardProps> = ({ title, value, change, changeType = 'neutral', children }) => {
  const changeColor = changeType === 'positive' ? 'text-positive' : changeType === 'negative' ? 'text-negative' : 'text-text-secondary';
  ...
  <div className="bg-bg-secondary p-5 rounded-2xl shadow-md border border-border-color flex flex-col items-center justify-center text-center card-hover cursor-pointer group overflow-hidden relative">
      ...
      {change && (
        <p className={`text-sm font-semibold ${changeColor} transition-all duration-300 group-hover:scale-105`}>
          {change}
        </p>
      )}
