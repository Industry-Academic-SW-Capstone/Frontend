...
import { CompetitionLog } from '@/lib/types';
...
const MOCK_LOGS: CompetitionLog[] = [
  ...
  },
  {
    ...
    metadata: { field: 'maxInvestmentPerStock', oldValue: 2000000, newValue: 3000000 },
  },
];

const CompetitionLogsTab: React.FC<CompetitionLogsTabProps> = ({ competitionId }) => {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-bg-secondary border border-border-color rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-text-primary">{MOCK_LOGS.length}</p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <p className="font-bold text-text-primary">{log.description}</p>
                ...
                {log.username && (
                  <p className="text-sm text-text-secondary">by {log.username}</p>
                )}
                {log.metadata && Object.keys(log.metadata).length > 0 && (
                  <div className="mt-2 p-2 bg-bg-primary rounded-lg text-xs">
                    ...
                    {log.type === 'setting_change' && (
                      <p className="text-text-secondary">
                        {log.metadata.oldValue?.toLocaleString()} → {log.metadata.newValue?.toLocaleString()}
                      </p>
                    )}
                  </div>
                )}
