          ...
          >
            {currentStep > step.id ? (
              ...
              <>
                ...
                <div
                  className="relative flex h-9 w-9 items-center justify-center rounded-full border-2 border-primary bg-bg-primary"
                  aria-current="step"
                >
                  <span
            ...
            <span
              className={`absolute top-10 left-1/2 -translate-x-1/2 text-xs font-semibold whitespace-nowrap ${
          ...
          <div className="space-y-6 bg-bg-secondary p-6 rounded-2xl">
            <div>
              ...
              <input
                ...
                placeholder="예: 제 1회 주린이 수익률 대회"
                className="w-full bg-bg-primary border border-border-color rounded-lg p-3 focus:ring-2 focus:ring-primary outline-none"
              />
            </div>
            <div>
              <label
        ...
        return (
          <div className="space-y-6 bg-bg-secondary p-6 rounded-2xl">
            <div>
              ...
              <div className="text-center font-bold text-2xl text-primary mb-2">
                {rules.startingCapital.toLocaleString()}원
              </div>
              <input
                type="range"
                min="1000000"
            ...
            <div>
              ...
              <input
                ...
                max="100"
                step="5"
                value={rules.maxInvestmentPerStock}
                onChange={(e) =>
                  setRules({
                    ...rules,
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-2">
                투자 허용 섹터{" "}
                <span className="text-xs">(미선택 시 전체 허용)</span>
              </label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-3 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${
                      rules.allowedSectors.includes(sector.name)
          ...
          <div className="space-y-6 bg-bg-secondary p-6 rounded-2xl">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setRules({ ...rules, startDate: e.target.value })
                }
                className="w-full bg-bg-primary border border-border-color rounded-lg p-3"
              />
            </div>
            <div>
              <label
              ...
              <input
                ...
                value={rules.endDate}
                onChange={(e) =>
                  setRules({ ...rules, endDate: e.target.value })
                }
                className="w-full bg-bg-primary border border-border-color rounded-lg p-3"
              />
            </div>
        ...
        return (
          <div className="text-center p-8 bg-bg-secondary rounded-2xl flex flex-col items-center">
            ...
            <p className="text-text-secondary mt-2">
              "{rules.name}" 대회가 성공적으로 생성되었습니다. 친구들을 초대해
              함께 즐겨보세요!
            </p>
          </div>
        );
    ...
    <div className="h-full bg-bg-primary flex flex-col overflow-hidden">
      ...
      <div className="p-4 border-t border-border-color bg-bg-primary">
        ...
        {step === 4 && (
          <button
            onClick={onBack}
            className="w-full bg-positive text-white font-bold py-3 px-4 rounded-lg hover:bg-positive/90 transition-colors"
          >
            완료
          </button>
        )}
