...
const LandingScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const isInStandalone = ('standalone' in window.navigator && (window as any).navigator.standalone) || (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
      ...
      <div className="mt-12 flex flex-col sm:flex-row gap-4">
        ...
        {showInstallButton && (
          ...
          >
            <svg className="w-5 h-5 text-slate-600" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
      ...
      {showIosModal && (
        <div className="fixed inset-0 z-40 flex items-end justify-center px-4 pb-8 sm:items-center sm:pb-0">
          <div className="absolute inset-0 bg-black/40" onClick={() => setShowIosModal(false)} aria-hidden />
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-6 text-left">
            <div className="flex items-start gap-4">
              <div className="flex-shrink-0 mt-1">
                <div className="w-10 h-10 bg-primary text-white rounded-xl flex items-center justify-center">+</div>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Install on iPhone</h3>
                <p className="text-sm text-slate-600 mt-2">Tap the share button in Safari (the {`⤴︎`} icon), then choose <strong className="text-slate-900">Add to Home Screen</strong>.</p>
