...
const MissionSummary: React.FC<{ onOpenMissions: () => void }> = ({ onOpenMissions }) => {
  const progress = MOCK_MISSION_PROGRESS;
  const dailyProgress = (progress.dailyMissionsCompleted / progress.dailyMissionsTotal) * 100;

  return (
    <div
      onClick={onOpenMissions}
      className="bg-linear-to-br from-primary to-secondary p-6 rounded-3xl text-white shadow-lg space-y-4 card-hover cursor-pointer overflow-hidden relative"
    >
      ...
      <div className="relative z-10">
        ...
        <div className="w-full bg-white/20 backdrop-blur-sm rounded-full h-3 shadow-inner overflow-hidden mb-4">
          <div
          ...
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-2 text-center">
              ...
              <p className="font-bold text-base">
                {progress.themeMissions.beginner.completed}/{progress.themeMissions.beginner.total}
              </p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-2 text-center">
              <p className="opacity-80 mb-1">중급</p>
              <p className="font-bold text-base">
                {progress.themeMissions.intermediate.completed}/{progress.themeMissions.intermediate.total}
              </p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-2 text-center">
              <p className="opacity-80 mb-1">고급</p>
              <p className="font-bold text-base">
                {progress.themeMissions.advanced.completed}/{progress.themeMissions.advanced.total}
              </p>
            </div>
          </div>
        </div>
...
const HomeScreen: React.FC<HomeScreenProps> = ({ selectedAccount }) => {
  ...
  return (
    <>
      ...
      {/* Mission Panel */}
      <MissionPanel isOpen={isMissionPanelOpen} onClose={() => setIsMissionPanelOpen(false)} />
    </>
  );
};
