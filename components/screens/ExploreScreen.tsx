...
const StockRow: React.FC<{ stock: BasicStockInfo, onClick: () => void }> = ({ stock, onClick }) => {
    const isPositive = stock.changePercent >= 0;
    return (
        <button onClick={onClick} className="w-full flex items-center justify-between p-4 rounded-xl hover:bg-border-color/50 transition-colors">
            <div className="flex items-center gap-3">
                <img src={stock.logo} alt={`${stock.name} logo`} className="w-10 h-10 rounded-full bg-white object-cover" />
                <div>
                    <p className="font-bold text-text-primary text-left">{stock.name}</p>
                    <p className="text-sm text-text-secondary text-left">{stock.ticker}</p>
                </div>
            </div>
            <div className="text-right">
                <p className="font-semibold text-text-primary">{stock.price.toLocaleString()}원</p>
                <div className={`flex items-center justify-end gap-1 text-sm font-semibold ${isPositive ? 'text-positive' : 'text-negative'}`}>
...
const PopularStockCard: React.FC<{ stock: BasicStockInfo, onClick: () => void }> = ({ stock, onClick }) => {
    const isPositive = stock.changePercent >= 0;
    return (
        <button onClick={onClick} className="flex-shrink-0 w-36 bg-bg-secondary border border-border-color rounded-2xl p-4 flex flex-col justify-between">
            <div>
                <img src={stock.logo} alt={stock.name} className="w-8 h-8 rounded-full mb-2 bg-white object-cover"/>
                <p className="font-bold text-sm text-text-primary truncate">{stock.name}</p>
            </div>
            <div>
                <p className="font-semibold text-text-primary">{stock.price.toLocaleString()}원</p>
                <p className={`font-semibold text-xs ${isPositive ? 'text-positive' : 'text-negative'}`}>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                <div className="flex border-b border-border-color">
                    <button onClick={() => setActiveContentTab('sectors')} className={`py-2 px-4 text-sm font-semibold ${activeContentTab === 'sectors' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`}>섹터별</button>
                    <button onClick={() => setActiveContentTab('favorites')} className={`py-2 px-4 text-sm font-semibold ${activeContentTab === 'favorites' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`}>관심 종목</button>
                </div>
                {activeContentTab === 'sectors' && (
                    <div className="space-y-4">
                        {filteredSectors.map(sector => (
                            <div key={sector.name}>
                                <h3 className="font-bold text-text-primary px-2 mb-1">{sector.name}</h3>
                                {sector.stocks.map(stock => (
                                    <StockRow key={stock.ticker} stock={stock} onClick={() => onSelectStock(stock.ticker)} />
                                ))}
                            </div>
                        ))}
                    </div>
                ...
                {activeContentTab === 'favorites' && (
                    <div className="space-y-2">
                        {filteredFavorites.length > 0 ? filteredFavorites.map(stock => (
                            <StockRow key={stock.ticker} stock={stock} onClick={() => onSelectStock(stock.ticker)} />
                        )) : (
