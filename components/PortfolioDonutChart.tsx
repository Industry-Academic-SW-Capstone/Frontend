...
const DonutChart: React.FC<PortfolioDonutChartProps> = ({ holdings, cash }) => {
  ...
  if (totalPortfolioValue === 0) {
    return <div className="text-center text-text-secondary">자산 데이터가 없습니다.</div>;
  }
  ...
  const data = [
    ...
    ...holdings.map((h, i) => ({
      ...
      color: ['#f87171', '#fb923c', '#facc15', '#4ade80', '#38bdf8', '#c084fc'][i % 6],
    })),
    ...
    <div className="bg-bg-secondary p-4 rounded-2xl border border-border-color flex flex-col md:flex-row items-center gap-6">
      <div className="relative w-40 h-40">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="font-bold text-lg text-text-primary">{totalPortfolioValue.toLocaleString()}원</span>
        </div>
      </div>
          ...
          {segments.map((item, index) => (
            <li key={index} className="flex items-center justify-between text-sm">
              ...
              <div className="font-semibold text-text-primary">
                ...
                <span className="text-xs text-text-secondary ml-2">({(item.percent * 100).toFixed(1)}%)</span>
              </div>
            </li>
          ))}
        </ul>
